...
from data.db import create_database, generate_master_key, table_columns
...
def test_cannot_reparent_division_referenced_by_employees(tmp_path: Path) -> None:
    ...
    with pytest.raises(sqlcipher.DatabaseError, match="cannot move division"):
        conn.execute("UPDATE divisions SET branch_id = 2 WHERE id = 1")
    branch = conn.execute("SELECT branch_id FROM divisions WHERE id = 1").fetchone()[0]
    assert branch == 1
    conn.close()
