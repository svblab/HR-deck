...
def _observer_session(conn: Connection, admin: SessionState, db: Path) -> SessionState:
    mgr = AccountManagementService(conn, admin, db_path=db, clock=lambda: "2026-08-30T10:01:00Z")
...
def test_archive_restore_hides_and_shows_in_roster(tmp_path: Path) -> None:
    ...
    archived_rows = [r for r in roster.list_rows(include_archived=True) if r.is_archived]
    assert any(r.employee_id == emp_id for r in archived_rows)
...
@pytest.mark.acceptance
def test_status_history_unchanged_on_archive(tmp_path: Path) -> None:
    conn, session, _db = _open_db(tmp_path)
    ids = seed_synthetic_org(conn)
    emp_id = ids["employee_a_id"]
    employees, history = _employees_with_history(conn, session, clock="2026-08-30T10:20:00Z")
    ...
    assert after_restore == before_archive
    review = conn.execute(
...
def test_archive_employee_sets_dismissed_employment_type(tmp_path: Path) -> None:
    conn, session, _db = _open_db(tmp_path)
    ids = seed_synthetic_org(conn)
    emp_id = ids["employee_a_id"]
    ...
    employees, history = _employees_with_history(conn, session, clock="2026-08-30T10:15:00Z")
    history.assign_status(emp_id, status_id=1, start_date="2026-08-01")
    ...
    assert archived == (1, dismissed_id)
...
def test_archive_employee_graceful_without_dismissed_type(tmp_path: Path) -> None:
    conn, session, _db = _open_db(tmp_path)
    ids = seed_synthetic_org(conn)
    emp_id = ids["employee_a_id"]
    before_type = conn.execute(
    ...
    assert archived == (1, before_type)
    conn.close()


def test_restore_employee_does_not_change_employment_type(tmp_path: Path) -> None:
    conn, session, _db = _open_db(tmp_path)
    ids = seed_synthetic_org(conn)
    emp_id = ids["employee_a_id"]
    dismissed_id = _ensure_dismissed_employment_type(conn)
    ...
    row = conn.execute(
        "SELECT employment_type_id, needs_org_review FROM employees WHERE id = ?",
        (emp_id,),
    ).fetchone()
    assert row == (dismissed_id, 1)
    conn.close()
...
def test_mark_needs_org_review_preserves_department_and_division(
    ...
    EmployeeRepository(conn).mark_needs_org_review(emp_id, updated_at="2026-08-30T10:16:00Z")
    conn.commit()
    after = conn.execute(
...
def test_restored_employee_appears_in_org_review_roster_filter(tmp_path: Path) -> None:
    ...
    ids = seed_synthetic_org(conn)
    emp_id = ids["employee_a_id"]
    employees, _history = _employees_with_history(conn, session, clock="2026-08-30T10:17:00Z")
...
def test_search_by_name_stays_active_only(tmp_path: Path) -> None:
    """Duplicate-hint search ignores archived employees."""
    conn, session, _db = _open_db(tmp_path)
    ids = seed_synthetic_org(conn)
    emp_id = ids["employee_a_id"]
    card = conn.execute("SELECT full_name FROM employees WHERE id = ?", (emp_id,)).fetchone()[0]
    prefix = card[:4]
